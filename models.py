...
from definitions import BALL_RADIUS, BALL_HUE, BALL_HUE_SPEED
from definitions import BALL_SOUND_FILEPATH
from definitions import BALL_APPLY_SPEED_FACTOR
...
class Ball():
    ...
    def dist_from_center(self):
        return sqrt(
                ((WINDOW_WIDTH//2)-self.posx)**2
                + (self.posy - (WINDOW_HEIGHT//2))**2
                )

    def reflect(self):
        vel = sqrt(self.velx**2 + self.vely**2)
        step = 0.2
        while self.dist_from_center() > BOUNDARY_RADIUS - self.radius:
